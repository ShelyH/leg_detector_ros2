...
void SampleSet::clear() {
    for (SampleSet::iterator i = begin(); i != end(); ++i) delete (*i);
...
ScanProcessor::~ScanProcessor() {
    for (std::list<SampleSet*>::iterator c = clusters_.begin(); c != clusters_.end(); ++c) delete (*c);
}
...
void ScanProcessor::splitConnected(float thresh) {
    ...
    while (c_iter != clusters_.end()) {
        while ((*c_iter)->size() > 0) {
            ...
            for (s_q = sample_queue.begin(); s_q != sample_queue.end(); s_q++) c->insert(*s_q);
